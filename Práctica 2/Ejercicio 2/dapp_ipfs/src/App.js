...
function App() {
  ...
  // Maneja el cambio de archivo para la creación de NFT
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title || !description) {
      alert('Please provide a title and description for the Cryptomon.');
      return;
    }
    ...
    try {
      ...
      alert(`Cryptomon created successfully! Metadata CID: ${metadataCID}`);
    } catch (error) {
      console.error('Error creating Cryptomon:', error);
      ...
      alert('Error occurred during Cryptomon creation. Resources cleaned up where possible.');
    }
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
        ...
        return (
          <div className="create-nft-form">
            ...
            <form onSubmit={handleSubmit}>
              ...
              />
              <textarea
                placeholder="Cryptomon Description (used to generate image)"
                value={description}
      ...
      case 'my-nfts':
        return <MyCryptomons />; // Usar el nuevo componente aquí
      default:
